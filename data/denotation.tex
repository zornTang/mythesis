...
\nomenclature[atp6v1b2]{ATP6V1B2}{V-ATPase V1 结构域 B2 亚基}
...
\nomenclature[bert]{BERT}{基于 Transformer 的双向编码器表征 (Bidirectional Encoder Representations from Transformers)}
...
\nomenclature[crispr-cas9]{CRISPR-Cas9}{成簇规律间隔短回文重复序列及其相关蛋白 9 (Clustered Regularly Interspaced Short Palindromic Repeats-CRISPR associated protein 9)}
...
\nomenclature[mageck]{MAGeCK}{基于模型的全基因组 CRISPR-Cas9 敲除分析方法 (Model-based Analysis of Genome-wide CRISPR-Cas9 Knockout)}
...
\nomenclature[plbd1]{PLBD1}{磷脂酶 B 结构域包含蛋白1 (Phospholipase B Domain-containing 1)}
...
\nomenclature[prott5]{ProtT5}{基于 T5 架构的预训练蛋白语言模型}
...
\nomenclature[sgrna]{sgRNA}{单导 RNA (single-guide RNA)}
\nomenclature[shrna]{shRNA}{短发夹 RNA (short hairpin RNA)}
\nomenclature[t5]{T5}{文本到文本迁移 Transformer (Text-to-Text Transfer Transformer)}
\nomenclature[tape]{TAPE}{蛋白嵌入任务评估基准 (Tasks Assessing Protein Embeddings)}
\nomenclature[v-atpase]{V-ATPase}{液泡型 ATP 酶 (Vacuolar-type ATPase)}
