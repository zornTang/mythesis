...
为进一步判断单头结构的优势是否具有稳定性，而不仅仅体现在单一指标的静态比较上，还需结合训练动态与测试集曲线形态进行综合分析。图~\ref{fig:attention_heads_ablation} 从验证集收敛过程与测试集判别边界两个层面展示了不同 head 数模型的表现：前者用于比较各模型在训练过程中的提升速度与稳定性，后者用于观察在不同决策阈值下 Precision、Recall 与假阳性控制之间的整体权衡。若多头结构确实能够带来更强的特征表达能力，则其优势应同时体现在验证集动态演化与测试集曲线包络上；反之，则说明在当前任务中更复杂的头部拆分并未转化为稳定的泛化收益。结合图中各曲线的整体走势可以预期，单头模型不仅在训练早期即达到较高性能水平，而且在后期仍保持相对稳定的领先优势；相比之下，多头模型虽然参数容量更高，但并未表现出持续的性能增益。与此同时，测试集 PR 与 ROC 曲线的相对位置也为这一判断提供了直观支持，即单头结构在高召回区间仍能维持更高的 Precision，说明其在不同判别阈值下具有更优的区分能力。综合来看，在当前任务设定下，多头注意力机制未带来有效的表示空间分解优势。预训练 PLM 嵌入已编码多维语义与结构约束信息，单头注意力足以完成任务驱动的关键残基加权与全局依赖重构。增加 head 数可能导致特征子空间过度分解，从而削弱全局一致性，并在有限数据规模下引入冗余参数与潜在过拟合风险。因此，综合验证集与测试集结果，本研究最终采用单头 Attention 作为默认配置。
